...
public class DBScan {
    ...
    protected double epsilon = 0.07;

    //Specifies the next parameter: the density (the range-query must contain at least minPoints DataObjects)
    protected int minPoints = 6;

    //Holds the number of clusters generated
    protected int numberOfGeneratedClusters;

    //Holds the distance-type that is used
    protected static String distanceMeasure = "solap.clustering.support.distancesmeasures.EuclidianClusterObject";
    ...
    protected int processed_InstanceID;
    protected double elapsedTime;

    public DBScan() {
    }
    
    public IDatabase initializeDatabase(CollectionSpatialObjects instances) {
    ...
    public static List<Double> sortedKDistGraph (CollectionSpatialObjects instances, int k) {
        ...
        IDatabase database = new Database(instances);
        distanceMeasure = "solap.clustering.support.distancesmeasures.EuclidianClusterObject";
        for (int i = 0; i < database.getInstances().numberInstances(); i++) {
            ClusterObject dataObject = dataObjectByDistanceMeasure(database.getInstances().getInstance(i),
                    Integer.toString(i),
                    (Database) database);
    ...
    private static PriorityQueue firstKNeighborhoods(int k, ClusterObject clusterObject, IDatabase spatialObjects) {
        ...
        while(it.hasNext()) {
            ...
            //double dist = Distances.computeEuclidianDistance(clusterObject.getInstance().getPoint(), obj.getInstance().getPoint() ) ;
